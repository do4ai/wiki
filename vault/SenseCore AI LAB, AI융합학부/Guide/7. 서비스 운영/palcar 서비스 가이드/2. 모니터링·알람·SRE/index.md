...
- 보는 순서: Grafana(5xx·리소스) → Kibana(로그) → Tempo(`palcar-api` 스팬). 전체 흐름은 [모니터링·알림 아키텍처 가이드](../../../8. 데이터, 관측, 보안/모니터링·알림 아키텍처 가이드/index.md).
...
- **확장**: 기존 1 replica → HPA로 prod 최소 2 replica(HA 개선). 개념은 [SLO·SLI와 에러 버짓 가이드](../../../10. 장애 대응과 운영 판단/SLO·SLI와 에러 버짓 가이드/index.md).
