...
- **보는 순서**: 메트릭(범위) → 로그(원인) → 트레이스(병목). 전체 파이프라인은 [모니터링·알림 아키텍처 가이드](../../../3. 데이터, 관측, 보안/모니터링·알림 아키텍처 가이드/index.md).
...
- 개념·기준은 [SLO·SLI와 에러 버짓 가이드](../../../4. 장애 대응과 운영 판단/SLO·SLI와 에러 버짓 가이드/index.md).
