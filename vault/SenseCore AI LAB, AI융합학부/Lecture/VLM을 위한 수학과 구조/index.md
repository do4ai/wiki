...
이 교본은 새 커리큘럼에서 C8 비전·언어 모델의 수학(183~205강)이 됩니다. 이미지 좌표계와 아핀 변환, 대조학습, 교차 어텐션, 확산 손실, 그리고 브래들리 테리 모형에서 DPO 목적함수까지 이어지는 정렬의 수학을 다룹니다. 데이터 믹스처와 벤치마크 운용, 서빙 비용, 안전 정책은 수식으로 유도할 수 없으므로 수학 트랙에서 분리해 별도 실무 교본으로 뺍니다. 전체 학습 순서는 [00. 학습 순서와 커리큘럼](../00. 학습 순서와 커리큘럼/index.md)에 있습니다.
