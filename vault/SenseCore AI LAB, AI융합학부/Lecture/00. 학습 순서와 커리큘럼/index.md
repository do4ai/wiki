...
이 문서는 SCAI LAB 전체 커리큘럼의 설계도입니다. 기존 교본 폴더를 정리한 목록이 아니라, 선수 관계만 따져 처음부터 설계한 학습 순서입니다. 중학 수학을 마친 사람이 1강부터 383강까지 번호 순서대로 따라가기만 하면 되도록 짰습니다. 개별 과목의 index는 이 문서의 넘버링을 그대로 따릅니다.

# 이 커리큘럼이 약속하는 것

중학 수학까지 아는 사람이 1강부터 순서대로 읽고 문제를 풀면, 마지막 383강에서 멀티모달 논문의 손실함수를 스스로 유도할 수 있는 상태가 됩니다. 중간에 다른 책을 찾아볼 필요가 없도록, 뒤에서 쓰는 도구는 반드시 앞에서 문제로 끌어냅니다.
...
강의 번호는 과목이 바뀌어도 초기화하지 않고 1번부터 383번까지 계속 이어집니다. 학습자는 어떤 책을 열지 고르거나 순서를 판단할 필요가 없습니다. 지금 몇 번 강의까지 풀었는지만 알면 다음에 무엇을 열지가 결정됩니다. 과목과 트랙은 내용을 묶어 부르는 이름일 뿐이며 학습 단위가 아닙니다.
...
정답이 하나로 떨어지지 않는 강의가 일부 있습니다. 시각화 진단(172강), 특성공학(228강), 보상 설계(290강), 벤치마크 설계(343강)가 그렇습니다. 이런 강의는 심화 문제 자리에 **판단 문제**를 둡니다. 판단 문제는 선택지마다 근거와 반례를 함께 제시해 왜 그 선택이 나은지를 묻습니다.
...
과목 하나는 12강에서 37강으로 길고 전체는 383강이므로, 다음 단계로 넘어가도 되는지를 스스로 판단하기 어렵습니다. 그래서 아홉 곳에 **관문**을 둡니다. 관문은 새 내용을 가르치지 않고, 앞에서 배운 것을 섞어 60~80문항으로 다시 묻기만 합니다. 정답률 80퍼센트를 넘지 못하면 지정된 단원으로 돌아갑니다.
...
- 트랙 번호는 `T1`~`T7`, 과목 번호는 `S1`~`S15`를 쓰며, 폴더 이름에는 넣지 않고 이 문서와 각 과목 index에서만 표기합니다.
- 단원 폴더는 과목 안에서 `01`부터 두 자리로 매깁니다. 예를 들어 `03. 미분의 활용`입니다.
- 강의 폴더는 **과목을 가로질러 1번부터 383번까지 연속**으로 매기며 자리수를 채우지 않습니다. 예를 들어 `62. 벡터와 벡터 연산`입니다.
...
행렬을 숫자 표가 아니라 공간을 옮기는 변환으로 읽습니다. 마지막 단원에서 계산이 언제 믿을 수 없게 되는지를 다루며, 이 내용은 그래디언트 폭발(246강)과 플래시어텐션(306강)에서 다시 쓰입니다.
...
불확실성을 수로 다루고, 데이터에서 모수를 추정하고 판단하는 절차를 세웁니다. 마지막 단원의 확률과정은 강화학습(274강)과 확산모형(266강)의 공통 기반이 됩니다.
...
데이터를 분류하는 것에서 데이터를 만들어 내는 것으로 넘어갑니다. 여기의 확산모형은 이미지 생성(371강)에서 그대로 쓰입니다.
...
- **3차원 시각 정보**를 다루지 않습니다. 카메라 기하, 에피폴라 기하, NeRF, 3차원 가우시안 스플래팅은 별도 트랙이 필요할 만큼 크고, 비전·언어 모델 논문 독해에는 필수가 아닙니다.
- **음성을 전문적으로** 다루지 않습니다. 380강에서 개요만 보며, 스펙트로그램, CTC, 신경 음성 코덱은 다루지 않습니다.
- **하드웨어와 커널 최적화**를 다루지 않습니다. 306강에서 플래시어텐션의 계산 구조는 유도하지만 CUDA 커널 작성은 범위 밖입니다.
- **MLOps와 서비스 운영**을 다루지 않습니다. 실험 추적, 모델 레지스트리, 배포 파이프라인은 풀스택실전스터디 교본이 맡습니다.
- **AI 규제와 정책**을 다루지 않습니다. 수식으로 유도할 수 있는 대상이 아니므로 이 트랙의 문제 형식에 맞지 않습니다.
...
| 기하학 | 해체 | 삼각비와 삼각함수는 15~17강, 벡터기하는 62~65강, 미분기하는 102강, 변환기하는 352강으로 나누어 심습니다. |
...
S12 강화학습은 기존 교본에 대응하는 자리가 없으므로 전부 새로 씁니다.
...
전체 분량은 383개 강의, 80권의 단원 문제집, 9개의 관문입니다. 강의당 개념 문제 5~6개, 종합 드릴 10~13개, 심화 4~6개를 기준으로 하면 강의 안 문제가 약 8,400문항이고 단원 문제집이 약 3,500문항이며 관문이 약 630문항이므로, 합쳐서 약 12,500문항 규모가 됩니다.
